...
class LLMSingleton:
    ...
    @classmethod
    def _is_lora_adapter(cls, model_path):
        #Auto-detect if path contains a LoRA adapter
        if not os.path.isdir(model_path):
            return False

        adapter_files = ['adapter_config.json', 'adapter_model.bin', 'adapter_model.safetensors']
        return any(os.path.exists(os.path.join(model_path, f)) for f in adapter_files)

    @classmethod
    def _get_base_model_name(cls, model_path):
    ...
    def get_model(cls, model_name):
        with cls._models_lock:
            if model_name not in cls._models:
                ...
                if cls._is_lora_adapter(model_name):
                    base_model_name = cls._get_base_model_name(model_name)
                    ...
                    # Load regular model
                    print("Model loaded without adapter")
                    model = AutoModelForCausalLM.from_pretrained(model_name)
                    cls._models[model_name] = model
    ...
    def get_tokenizer(cls, tokenizer_name):
        with cls._tokenizers_lock:
            if tokenizer_name not in cls._tokenizers:
                ...
                if cls._is_lora_adapter(tokenizer_name):
                    # Try to load tokenizer from adapter path first
                    tokenizer_files = ['tokenizer.json', 'tokenizer_config.json', 'vocab.json']
                    has_tokenizer = any(os.path.exists(os.path.join(tokenizer_name, f)) for f in tokenizer_files)

                    if has_tokenizer:
                        logging.debug("Loading tokenizer from LoRA adapter path")
                        tokenizer = AutoTokenizer.from_pretrained(tokenizer_name)
                    else:
                        # Fallback to base model tokenizer
                        base_model_name = cls._get_base_model_name(tokenizer_name)
                        logging.debug(f"Loading tokenizer from base model: {base_model_name}")
...
def new_decoding_task_init(self, model: Whisper, options: DecodingOptions):
    ...
    import dataclasses
    options = dataclasses.replace(options, beam_size=5)
    print(options)
...
DecodingTask.__init__ = new_decoding_task_init
print("Whisper DecodingTask has been monkey patched!")
